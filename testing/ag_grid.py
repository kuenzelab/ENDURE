...
from st_aggrid import (
    GridOptionsBuilder, AgGrid, GridUpdateMode, DataReturnMode, JsCode
)
...
return_mode = st.sidebar.selectbox(
    label="Return Mode",
    options=list(DataReturnMode.__members__),
    index=1
)
...
update_mode = st.sidebar.selectbox(
    label="Update Mode",
    options=list(GridUpdateMode.__members__),
    index=6
)
...
if enable_enterprise_modules:
    enable_sidebar = st.sidebar.checkbox(
        label="Enable grid sidebar",
        value=False
    )
else:
...
enable_selection = st.sidebar.checkbox(
...
if enable_selection:
    ...
    use_checkbox = st.sidebar.checkbox(
        label="Use check box for selection",
        value=True
    )
    if use_checkbox:
        groupSelectsChildren = st.sidebar.checkbox(
            label="Group checkbox select children",
            value=True
        )
        groupSelectsFiltered = st.sidebar.checkbox(
            label="Group checkbox includes filtered",
            value=True
        )

    if (selection_mode == 'multiple') & (not use_checkbox):
        rowMultiSelectWithClick = st.sidebar.checkbox(
            label="Multiselect with click (instead of holding CTRL)",
            value=False
        )
        if not rowMultiSelectWithClick:
            suppressRowDeselection = st.sidebar.checkbox(
                label="Suppress deselection (while holding CTRL)",
                value=False
            )
        else:
...
enable_pagination = st.sidebar.checkbox(
    label="Enable pagination",
    value=False
)
if enable_pagination:
    st.sidebar.subheader("Pagination options")
    paginationAutoSize = st.sidebar.checkbox(
        "Auto pagination size",
        value=True
    )
...
# Configures last row to use custom styles based on cell's value
